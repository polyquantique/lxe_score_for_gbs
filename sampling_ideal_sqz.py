"""
    Generates a sample from an ideal squeezed state model 
    and computes its conditional probability with respect to the same model.
"""
...
def uniform_sqz_cov(num_modes, squeezing_par, hbar=2):
    """Computes the covariance matrix of num_modes squeezed states with the same squeezing parameter
    ...
    Returns:
        (array): real covariance matrix of the multimode squeezed state

    """
    x_diag_elems = [np.exp(-2 * squeezing_par)] * num_modes
    p_diag_elems = [np.exp(2 * squeezing_par)] * num_modes
...
parser = argparse.ArgumentParser(
    description="Generate a sample from the ideal squeezed state model distribution"
)
...
sqz_cov = uniform_sqz_cov(num_modes=args.M, squeezing_par=sqz_par)
